...
import { streamChat, type SuggestedBuild, type ServiceBundle } from '../../engine/chatService';
...
function ApplyBuildCard({ build, onApply }: { build: SuggestedBuild; onApply: () => void }) {
  ...
  const colorName = build.bodyColor ? getColor(build.bodyColor)?.name : undefined;
  const finishName = build.finishType ? getFinish(build.finishType)?.name : undefined;
        ...
        {colorName && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-zinc-400">
              {colorName}{finishName ? ` · ${finishName} finish` : ''}
            </span>
          </div>
        )}
        {modNames.length > 0 && (
...
export function AIChatbot() {
  ...
  // Fixed position: just to the left of the right panel, below the header
  // Right panel is w-96 (384px); button sits 8px to its left
  const BTN_RIGHT = 384 + 8; // px from right edge of viewport
  const BTN_TOP = 68;        // px from top (below header)
  ...
  function applyBuild(build: SuggestedBuild) {
    if (build.bodyColor && getColor(build.bodyColor)) setBodyColor(build.bodyColor);
    if (build.finishType && getFinish(build.finishType)) setFinishType(build.finishType as FinishType);
    if (build.rimColor && getColor(build.rimColor)) setRimColor(build.rimColor);
    if (build.caliperColor && getColor(build.caliperColor)) setCaliperColor(build.caliperColor);

    // Only apply mod IDs that exist in the catalog
    const validMods = build.selectedMods.filter((id) => !!getMod(id));
    const toAdd = validMods.filter((id) => !customization.selectedMods.includes(id));
    const toRemove = customization.selectedMods.filter((id) => !validMods.includes(id));
  ...
  async function sendMessage(text: string) {
    ...
    // The assistant reply will be inserted at this fixed index
    const asstIdx = newMessages.length;
    ...
    const updateAt = (patch: Partial<Message>) =>
      setMessages((prev) => {
        const updated = [...prev];
        if (updated[asstIdx]) updated[asstIdx] = { ...updated[asstIdx], ...patch };
        return updated;
      });
    ...
    try {
      await streamChat(
        ...
        (chunk) => setMessages((prev) => {
          const updated = [...prev];
          if (updated[asstIdx]) updated[asstIdx] = { ...updated[asstIdx], content: (updated[asstIdx].content ?? '') + chunk };
          return updated;
        }),
        () => setLoading(false),
        (build) => { console.log('[AIChatbot] suggestedBuild received:', build); updateAt({ suggestedBuild: build }); },
        abortRef.current.signal
      );
    } catch (err) {
      if (err instanceof Error && err.name === 'AbortError') {
        // Request was superseded by a new message — remove the empty bubble
        setMessages((prev) => prev.filter((_, i) => i !== asstIdx));
        setLoading(false);
        return;
      }
      console.error('[AIChatbot] streamChat error:', err);
      updateAt({ content: `Sorry, I had trouble connecting. Error: ${err instanceof Error ? err.message : String(err)}` });
      setLoading(false);
    }
  }
  ...
  return (
    <>
      {/* Fixed floating button + label */}
      <div
        className="fixed z-60 select-none"
        style={{ right: BTN_RIGHT, top: BTN_TOP, width: BTN }}
      >
        {/* Label below button — hidden when chat is open */}
        {!open && (
          <div
            className="absolute top-full mt-2 left-1/2 -translate-x-1/2 whitespace-nowrap px-2.5 py-1 rounded-full text-xs font-semibold text-black shadow-lg pointer-events-none"
        ...
        <button
          onClick={() => setOpen((o) => !o)}
          className="w-14 h-14 bg-orange-500 hover:bg-orange-400 rounded-full flex items-center justify-center shadow-lg transition-colors cursor-pointer"
          title="Open AI Chat"
        >
          {open ? (
          ...
          <div className="px-3 py-2 border-b border-zinc-700 bg-zinc-800">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none" style={{ scrollbarWidth: 'none' }}>
              {combos.slice(0, 3).map((combo, i) => (
                <button
                  key={i}
                  onClick={() => sendMessage(`Tell me about the ${combo.bundle} combo`)}
